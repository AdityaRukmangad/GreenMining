...
============================
Train and evaluate baseline ML models for GreenMining CFD datasets.
...
# Potential leakage / weak features
OPTIONAL_DROP = [
    "Scenario",
]
...
def prepare_features(df, target_col):
    ...
    drop_cols = DROP_COLUMNS.copy()

    if target_col != "hazard_binary":
        drop_cols.append("hazard_binary")

    if target_col != "hazard_3class":
        drop_cols.append("hazard_3class")

    drop_cols += OPTIONAL_DROP

    drop_cols = [c for c in drop_cols if c in df.columns]

    X = df.drop(columns=drop_cols)

    # Handle categorical zone
...
def save_model(model, path):
    ...
    print(f"    Saved {path.name:<40} {size_mb:8.1f} MB")
...
def evaluate_model(
    ...
    print(f"\nEvaluating: {model_name} ({task_name})")
...
def save_feature_importance(
    ...
    ax.set_title(
        f"Feature Importance — {model_name} ({task_name})"
    )
...
def main(train_path, val_path, test_path):
    ...
    print("=" * 72)
    print("  GreenMining — Phase 5: Baseline ML Models")
    print("=" * 72)
    ...
    print("\nMost important metrics to inspect:")
    print("  - DANGER recall")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Phase 5 — Baseline ML Models"
    )
